...
1. In the GitHub Copilot chat window, use natural language to explain what you want to do. The MCP server determines which configured tool to use, and then shows it to you along with other information. Click **Continue** to run the tool and see the results of your request. To reduce the number of times you must subsequently click **Continue**, use the dropdown to automatically allow the tool to run in the session, workspace, or always. 
